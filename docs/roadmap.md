...
- **[CRITICAL COUPLING NOTE]**: This represents a major structural change. Implementing Iteration 5 alone breaks runtime safety since existing use cases expect a thrown exception. To prevent regressions, Iteration 5 and Iteration 6 must be implemented and validated in the same continuous delivery branch.
- **Prompt for /speckit.specify**:
  ```text
...
**Spec Module(s)**: [plant.md](spec/modules/plant.md), [validation.md](spec/modules/validation.md)
...
**Spec Module(s)**: [persistence.md](spec/modules/persistence.md)
...
- **Prompt for /speckit.specify**:
  ...
  WHAT: Allow GET pathways for Plants/Families to bypass aggregate hydration. Validate responses using strict Zod schemas supporting union types and polymorphic oneOf relations.
  ```

## Iteration 19: Support JSON:API Sparse Fields in Query Parser

**Spec Module(s)**: [query-dsl-contract.md](spec/modules/query-dsl-contract.md), [api-layer.md](spec/modules/api-layer.md), [openapi.md](spec/modules/openapi.md)
...
# Phase 1: Spatial Engine Prep

## Iteration 20: Extract PlantInstance into Standalone Collection

**Spec Module(s)**: [plant-instance.md](spec/modules/plant-instance.md), [bed.md](spec/modules/bed.md), [modules-map.md](spec/modules/modules-map.md)
...
- **Definition of Done**: `PlantInstances` are extracted from the `Bed` aggregate into a standalone `plant_instances` collection with its own repository. Crucially, position coordinates are defined as optional/nullable from day one inside schemas, mappers, and domain entities to prevent breaking database changes when implementing Nurseries.
...
- **Prompt for /speckit.specify**:
  ...
  WHAT: Extract PlantInstance into its own aggregate, collection, and repository, designing the position field as optional/nullable. Write a migrate-mongo script for existing embedded data.
  ```

## Iteration 21: Standardize Soft Deletion on Plant Aggregate

**Spec Module(s)**: [plant.md](spec/modules/plant.md)
...
## Iteration 22: Standardize Soft Deletion on Bed and PlantInstance

**Spec Module(s)**: [plant-instance.md](spec/modules/plant-instance.md), [bed.md](spec/modules/bed.md)
...
- **Dependencies**: Iteration 20, Iteration 21.
...
## Iteration 24: Wrap Cross-Aggregate Mutations in ACID Transactions

**Spec Module(s)**: [bed.md](spec/modules/bed.md), [plant-instance.md](spec/modules/plant-instance.md)
...
## Iteration 25: Implement Mailer Port and Local Bypass

**Spec Module(s)**: [auth.md](spec/modules/auth.md)
...
## Iteration 26: Implement Email Account Activation Flow
...
## Iteration 27: Implement Stateless Google OAuth
...
## Iteration 28: Implement Stateless GitHub OAuth
...
## Iteration 29: Add Geographic Fields to User Profile and Bed Aggregate

**Spec Module(s)**: [auth.md](spec/modules/auth.md), [bed.md](spec/modules/bed.md), [domain-core.md](spec/modules/domain-core.md)

- **Value delivered**: Sets the schema and inheritance foundation for localized, climate-aware agricultural rules.
- **Definition of Done**: User profile stores `postalCode`, `country`, `timezone`, and `hemisphere`. Additionally, Bed aggregate and MongoDB schema are updated to support `postalCode`, `country`, `timezone`, and `hemisphere`. The `CreateBed` usecase is refactored to fetch the User profile (or Redis cache) and copy/persist these fields as static, immutable properties inside the Bed document upon creation.
- **Dependencies**: None.
- **Risks**: Overlooking Bed aggregate schema changes during profile model updates.
- **Prompt for /speckit.specify**:
  ```text
  ADD GEOGRAPHIC FIELDS TO USER PROFILE AND BED AGGREGATE
  WHY: Storing geographic context in JWTs bloats headers. We need these fields on the User document and copied statically inside Bed documents on creation to guarantee immutable climate context history.
  WHAT: Add postalCode, country, timezone, and hemisphere to both User and Bed aggregates/schemas. Update CreateBed usecase to copy these fields.
  ```

## Iteration 30: Build Redis Cache Repository with Memory Fallback

**Spec Module(s)**: [auth.md](spec/modules/auth.md), [persistence.md](spec/modules/persistence.md)
...
# Phase 3: Taxonomy & Social

## Iteration 31: Introduce Collaborator Role in Auth Middleware

**Spec Module(s)**: [auth.md](spec/modules/auth.md), [family.md](spec/modules/family.md), [openapi.md](spec/modules/openapi.md), [plant-relation.md](spec/modules/plant-relation.md)
...
## Iteration 32: Implement DeleteFamily and Enable Polymorphic Lookups for Family Mutations
...
- **Prompt for /speckit.specify**:
  ```text
  IMPLEMENT DELETEFAMILY AND POLYMORPHIC LOOKUPS
  WHY: Supporting polymorphic id/slug updates simplifies API consumer usage. Complete administrative coverage requires implementing the missing DeleteFamily flow.
  WHAT: Write DeleteFamily usecase, add its express route, and refactor Family mutations (PATCH, DELETE) to support polymorphic idOrSlug.
  ```

## Iteration 33: Implement Private User Bookmarks Collection

**Spec Module(s)**: [auth.md](spec/modules/auth.md), [plant.md](spec/modules/plant.md)
...
## Iteration 34: Project Social Interaction Counters on Plant
...
# Phase 4: Botanical Enhancements

## Iteration 35: Make Plant Sowing Block Optional

**Spec Module(s)**: [plant.md](spec/modules/plant.md), [validation.md](spec/modules/validation.md), [openapi.md](spec/modules/openapi.md)
...
## Iteration 36: Apply Southern Hemisphere Sowing Calendar Shift

**Spec Module(s)**: [plant-instance.md](spec/modules/plant-instance.md), [bed.md](spec/modules/bed.md), [plant.md](spec/modules/plant.md)
...
## Iteration 37: Implement PlantRelation Aggregate and Repository

**Spec Module(s)**: [plant-relation.md](spec/modules/plant-relation.md), [modules-map.md](spec/modules/modules-map.md)
...
## Iteration 38: Expose PlantRelation CRUD Endpoints

**Spec Module(s)**: [plant-relation.md](spec/modules/plant-relation.md), [api-layer.md](spec/modules/api-layer.md)
...
- **Dependencies**: Iteration 31, Iteration 37.
...
# Phase 5: Organics & Diagnostics

## Iteration 39: Implement Anomaly Domain and CRUD Endpoints

**Spec Module(s)**: [knowledge.md](spec/modules/knowledge.md), [api-layer.md](spec/modules/api-layer.md), [openapi.md](spec/modules/openapi.md)
...
## Iteration 40: Implement GardenInput Domain and CRUD Endpoints

**Spec Module(s)**: [knowledge.md](spec/modules/knowledge.md)
...
## Iteration 41: Map Therapeutic Links in Anomaly Documents
...
- **Dependencies**: Iteration 39, Iteration 40.
...
## Iteration 42: Build Organic Dilution Calculator Service
...
**Spec Module(s)**: [observability-pipeline.md](spec/modules/observability-pipeline.md)
...
**Spec Module(s)**: [event-bus.md](spec/modules/event-bus.md)
...
**Spec Module(s)**: [event-bus.md](spec/modules/event-bus.md), [cultivation-logs.md](spec/modules/cultivation-logs.md)
...
- **Dependencies**: Iteration 24, Iteration 46.
...
**Spec Module(s)**: [seed-bank.md](spec/modules/seed-bank.md), [modules-map.md](spec/modules/modules-map.md), [openapi.md](spec/modules/openapi.md)
...
**Spec Module(s)**: [seed-bank.md](spec/modules/seed-bank.md)
...
**Spec Module(s)**: [spatial.md](spec/modules/spatial.md)
...
**Spec Module(s)**: [spatial.md](spec/modules/spatial.md), [bed.md](spec/modules/bed.md)
...
**Spec Module(s)**: [openapi.md](spec/modules/openapi.md), [bed.md](spec/modules/bed.md)
...
- **Definition of Done**: Batch save endpoint writes `BedLayoutSaved` event to outbox, which streams to Kafka. The event payload must explicitly segregate `addedInstances` (newly placed plants) from `movedInstances` (re-positioned existing plants) to prevent false depletions during mere spatial reorganization on the UI Canvas.
...
- **Prompt for /speckit.specify**:
  ...
  WHAT: Emit a consolidated `BedLayoutSaved` domain event during layout saves, segregating added and moved crop instances to trigger asynchronous inventory and reminder recalculations.
  ```

---

# Phase 12: Cultivation Logs (Garden Events)

## Iteration 70: Implement Append-Only CultivationLog Aggregate with Scopes

**Spec Module(s)**: [cultivation-logs.md](spec/modules/cultivation-logs.md)

- **Value delivered**: Provides a chronological agricultural log for tracking history.
- **Definition of Done**: Append-only `CultivationLog` entity implemented with explicit `scope: 'bed' | 'instance'`.
- **Dependencies**: None.
- **Risks**: None.
- **Prompt for /speckit.specify**:
  ```text
  IMPLEMENT APPEND-ONLY CULTIVATIONLOG AGGREGATE WITH SCOPES
  WHY: Tracking horticultural history is essential for diagnostic analysis. Scope-based logging avoids "magic nulls".
  WHAT: Create append-only CultivationLog entity. Validate scopes ('bed'|'instance').
  ```

## Iteration 71: Validate plantId Presence Rules per CultivationLog Type

**Spec Module(s)**: [cultivation-logs.md](spec/modules/cultivation-logs.md)

- **Value delivered**: Ensures precise agronomic logging (e.g., harvesting species vs watering soil).
- **Definition of Done**: Bed-scoped logs restrict `plantId` correctly (mandatory for harvest, optional for pruning, forbidden for watering).
- **Dependencies**: Iteration 70.
- **Risks**: None.
- **Prompt for /speckit.specify**:
  ```text
  VALIDATE PLANTID PRESENCE RULES PER CULTIVATIONLOG TYPE
  WHY: Watering targets soil (agnostic), but harvesting targets a specific species. The CultivationLog payload must enforce this.
  WHAT: Implement strict agronomic validation rules governing the presence of `plantId` in Bed-scoped logs based on the log type.
  ```

## Iteration 72: Expose CultivationLog CRUD Endpoints

**Spec Module(s)**: [cultivation-logs.md](spec/modules/cultivation-logs.md)

- **Value delivered**: Connects the UI to the chronological log engine.
- **Definition of Done**: CRUD endpoints for CultivationLogs exposed with query filter support.
- **Dependencies**: Iteration 71.
- **Risks**: None.
- **Prompt for /speckit.specify**:
  ```text
  EXPOSE CULTIVATIONLOG CRUD ENDPOINTS
  WHY: Users need API access to view and record agricultural journal entries.
  WHAT: Implement standard CRUD routes for CultivationLogs, supporting query DSL filtering by type, bedId, and plantInstanceId.
  ```

## Iteration 73: Implement Nested Soil Analysis Log on Bed

**Spec Module(s)**: [bed.md](spec/modules/bed.md), [cultivation-logs.md](spec/modules/cultivation-logs.md)
...
**Spec Module(s)**: [plant-instance.md](spec/modules/plant-instance.md), [validation.md](spec/modules/validation.md)
...
**Spec Module(s)**: [plant-instance.md](spec/modules/plant-instance.md), [api-layer.md](spec/modules/api-layer.md)
...
## Iteration 76: Decrement SeedBatch Stock and Handle Grace Period Refunds via Kafka Consumers

**Spec Module(s)**: [seed-bank.md](spec/modules/seed-bank.md), [plant-instance.md](spec/modules/plant-instance.md)

- **Value delivered**: Keeps inventory accurate without causing transaction bottlenecks, while preventing false deductions.
- **Definition of Done**: Asynchronous consumer decrements `remainingQuantity` upon receiving `BedLayoutSaved` or nursery sowing events. Specifically, it only decrements stock for IDs listed inside the `added` placements array (ignoring coordinate updates to prevent false depletions during mere spatial reorganization on the UI Canvas). It must also implement the Grace Period Refund Rule: if a crop instance is soft-deleted or its seed quantity is reduced within 24 hours of its creation (`createdAt` timestamp), the subscriber automatically refunds or increments the `remainingQuantity` of the linked `SeedBatch` (proportionately for quantity reductions).
...
- **Prompt for /speckit.specify**:
  ```text
  DECREMENT SEEDBATCH STOCK AND IMPLEMENT REFUNDS
  WHY: Stock updates should run asynchronously and be protected against drag-and-drop movements or accidental Canvas UI placement mistakes.
  WHAT: Build a Kafka event listener that decrements SeedBatch stock only for added placements and refunds seed stock (proportionately for quantity reductions) for soft-deletions under the 24-hour grace period.
  ```
...
**Spec Module(s)**: [cultivation-logs.md](spec/modules/cultivation-logs.md), [spatial.md](spec/modules/spatial.md)
...
- **Prompt for /speckit.specify**:
  ```text
  GENERATING ADVISORY WARNINGS FOR SOIL DEPLETION
...
**Spec Module(s)**: [reminders.md](spec/modules/reminders.md)
...
## Iteration 80: Reschedule Reminders and Cancel Treatments on Anomaly Recovery via Kafka Events

**Spec Module(s)**: [reminders.md](spec/modules/reminders.md)

- **Value delivered**: Keeps care schedules perfectly synced with real actions and automatically clears resolved issues.
- **Definition of Done**: Logging a cultivation event completes matching pending Reminders and schedules the next iteration asynchronously. Specifically, logging a recovery event must close active anomaly cycles and automatically cancel any recurring treatment reminders in cascade.
- **Dependencies**: Iteration 79, Iteration 72, Iteration 51.
- **Risks**: Race conditions in asynchronous scheduling.
- **Prompt for /speckit.specify**:
  ```text
  RESCHEDULE REMINDERS AND CANCEL TREATMENTS ON RECOVERY
  WHY: Care calendars must react to real logged actions and cancel ongoing chemical/organic treatments once an anomaly is resolved.
  WHAT: Build Kafka consumers that listen to Event creation, complete matching pending Reminders, and cancel recurring treatment reminders upon receiving a recovery event.
  ```
...
- **Dependencies**: Iteration 80, Iteration 22.
- **Risks**: None.
- **Prompt for /speckit.specify**:
  ```text
