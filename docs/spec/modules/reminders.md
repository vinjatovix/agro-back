...
## 1. PURPOSE `[TARGET STATE (Pending [Iteration 79](../../roadmap.md#iteration-79-implement-stateful-reminder-aggregate-and-crud))]`
...
## 5. PASSIVE EVENT-DRIVEN TRIGGERS `[TARGET STATE (Pending [Iteration 80](../../roadmap.md#iteration-80-reschedule-reminders-and-cancel-treatments-on-anomaly-recovery-via-kafka-events))]`
...
1. **PlantInstance Soft-Deletion `[TARGET STATE (Pending [Iteration 81](../../roadmap.md#iteration-81-dismiss-reminders-on-plantinstance-deletion))]`:**
...
2. **Bed Deletion or Empty Bed `[TARGET STATE (Pending [Iteration 82](../../roadmap.md#iteration-82-dismiss-reminders-on-bed-deletion))]`:**
...
- **Check `[TARGET STATE (Pending [Iteration 83](../../roadmap.md#iteration-83-integrate-open-meteo-forecasts-with-redis-geohash))]`:** Before displaying active watering reminders, the system checks cached weather reports. To prevent external API rate-limit exhaustion of Open-Meteo while preserving accurate local climate profiles, weather forecasts MUST be cached in Redis using a GeoHash key of exactly **length 5** (representing a spatial bounding box of approximately **4.8 km x 4.8 km**). This GeoHash length guarantees a high cache hit-rate across neighboring urban gardens while keeping the climatic resolution agronomically accurate.
- **Thundering Herd Protection (Promise Deduplication) `[TARGET STATE (Pending [Iteration 84](../../roadmap.md#iteration-84-implement-promise-deduplication-for-weather-fetching))]`:** To prevent thousands of simultaneous reminders scheduled at the exact same hour from triggering a mass concurrent fetch to Open-Meteo (if the Redis cache has expired), the system employs **Promise Deduplication (Lazy Fetching)**. If Redis is empty, the first Node.js thread initiates the API call and stores the pending `Promise` in a local memory Map. All subsequent concurrent requests for the same GeoHash will `await` this identical Promise, resulting in only 1 actual HTTP request to Open-Meteo, shielding the external API from rate limits.
- **Rule `[TARGET STATE (Pending [Iteration 85](../../roadmap.md#iteration-85-delay-irrigation-reminders-based-on-precipitation))]`:** If local precipitation exceeds a configurable threshold (e.g., 5mm of rain in the last 24h or forecasted for the next 12h), the system automatically silences or delays the reminder, updating its status to `'dismissed'` or rescheduling it with a note.
- **Protected Environment Bypass `[TARGET STATE (Pending [Iteration 86](../../roadmap.md#iteration-86-bypass-rain-delays-for-indoorgreenhouse-beds))]`:** This meteorological check is strictly **bypassed** under the centralized **Protected Environment Bypass Rule (spatial.md Sec. 5.4.1)** for `'indoor'` or `'greenhouse'` Bed environments. These protected crops are isolated from external precipitation and must never have their watering schedules silenced.
