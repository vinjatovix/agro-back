...
## 1. PURPOSE `[TARGET STATE (Pending Iterations [47](../../roadmap.md#iteration-47-write-domain-events-to-outbox-within-transactions), [48](../../roadmap.md#iteration-48-implement-mongodb-change-stream-tailer-for-outbox) & [50](../../roadmap.md#iteration-50-implement-consumer-idempotence-with-ttl-store))]`
...
This module is strictly isolated from **Agricultural Events** (`cultivation-logs.md`), which represent the user-logged horticultural log journal.
...
### 3.1 Transactional Outbox Pattern with Change Streams & Fallback Polling `[CRITICAL SECURITY RULE] [TARGET STATE (Pending [Iteration 46](../../roadmap.md#iteration-46-create-transactional-outbox-collection))]`
...
3.  **Fallback Polling Mechanism `[TARGET STATE]` (Oplog Rotation Resilience):** If the tailer process crashes or is taken offline for an extended period, the MongoDB Oplog may rotate and overwrite older entries, invalidating the Change Stream's `resume token`. If the tailer throws an `InvalidResumeToken` error upon startup:
    ...
    - It executes a standard query to retrieve stranded messages: `db.outbox.find({ processedAt: null }).sort({ _id: 1 })`. _(Note: Because `_id` maps directly to the domain's `eventId` encoded as a UUIDv7, sorting by `_id` guarantees perfect chronological sequencing without relying on external clocks or technical fields. To prevent full-table scans (COLLSCAN) and high-latency in-memory sorting under high write volumes during fallback polling, a composite index MUST be explicitly declared on `{ processedAt: 1, _id: 1 }` inside the outbox collection)._
...
### 3.2 Dynamic Partitioning & Message Ordering (Dual Design) `[TARGET STATE (Pending [Iteration 49](../../roadmap.md#iteration-49-route-events-to-kafka-domain-topic-by-partition-key))]`
...
- **Handling Out-of-Order Retries (Version/Timestamp Guards):** Because the retry mechanism is non-blocking (failing events are sent to progressive retry topics while the main partition offset is committed and continues processing), newer events might be processed before an older failed event finishes its retries. To prevent old events from overwriting newer state (e.g., a delayed 'watering' event overwriting the reminders of a later 'harvest' event), consumers of state-updating events MUST implement **Version/Timestamp Guards**. The consumer compares the incoming event's `occurredAt` or `id` (UUIDv7) with the timestamp of the last processed event recorded on the target aggregate/read-model; if the incoming event is older than the latest processed state, its state mutation is safely skipped/discarded while still being acknowledged.
...
- **Mechanism:** Consumers MUST atomically claim the incoming event ID in a uniquely indexed processed-events store and apply the resulting state mutation in the same MongoDB transaction; duplicate-key claims (violating the unique constraint) are treated as already processed, and the transaction is safely committed or ignored without executing side effects twice.
...
### 3.4 Non-Blocking Multi-Topic Consumer Retry Flow `[TARGET STATE (Pending [Iteration 51](../../roadmap.md#iteration-51-build-progressive-retry-and-dlq-consumer-flows))]`
...
### 3.6 BedLayoutSaved Consolidated Event Integration (KonvaJS Side-Effects) `[TARGET STATE (Pending [Iteration 69](../../roadmap.md#iteration-69-publish-bedlayoutsaved-event-to-kafka))]`
