...
- **Target State (Pending [Iteration 20](../../roadmap.md#iteration-20-extract-plantinstance-into-standalone-collection)):** `PlantInstance` will be re-architected as a standalone aggregate root (`PlantInstance` with its own `id`) with its own dedicated MongoDB collection (`plant_instances`), separate repository interface, and decoupled use cases.
...
- `seedBatchId`: **`[TARGET STATE (Pending [Iteration 52](../../roadmap.md#iteration-52-implement-seedbatch-aggregate-and-repository))]`** (optional UUID reference pointing to the user's private `SeedBatch`)
- `establishment`: **`[TARGET STATE (Pending [Iteration 74](../../roadmap.md#iteration-74-support-null-coordinates-for-nursery-establishments))]`** `direct` | `nursery` (represents how it was established, aligned with the biological catalog's methods). _Migration Note: When this schema change is implemented in code ([Iteration 74](../../roadmap.md#iteration-74-support-null-coordinates-for-nursery-establishments)), a MongoDB migration MUST be created to rename existing `sowingMethod` fields to `establishment`, and to map existing `'starter'` values to `'nursery'`._
- `position`: Coordinates object `(x, y)`
- `growthStatus`: (germinating, seedling, vegetative, flowering, fruiting, harvesting, dormant, dead)
- `status`: `'active'` | `'removed'` _(Migration Note: Currently implemented in code as `instanceStatus` with enum `PlantInstanceLifecycleStatus`. Must be migrated to `status` in [Iteration 22](../../roadmap.md#iteration-22-standardize-soft-deletion-on-bed-and-plantinstance))._
- `warnings`: (array of pre-computed and persisted spatial/biological alerts). To secure high-performance reads on the Canvas map, these alerts are evaluated and stored during save/placement write mutations. When calculating seasonality checks (verifying if sowing, transplanting, flowering, or harvesting is occurring in appropriate calendar months), the system inherits the `hemisphere` from the owning User's profile. **`[TARGET STATE (Pending [Iteration 36](../../roadmap.md#iteration-36-apply-southern-hemisphere-sowing-calendar-shift))]`** If the hemisphere is `south`, the sowing, flowering, and harvest calendars of the `Plant` aggregate (if present) are shifted by 6 months dynamically in-memory during validation using the 1-indexed circular arithmetic shift formula: `shiftedMonth = ((month - 1 + 6) % 12) + 1`. This guarantees correct circular month rotation and prevents month 6 (June) from being incorrectly evaluated as month 0, which would violate Zod calendar schema boundaries. If a plant definition lacks a `sowing` block, the sowing seasonality warning is automatically bypassed.
- `plantedAt`: ISODate
- `deletedAt`: (optional ISODate / null) _(Migration Note: Currently implemented in code as `removedAt`. Must be migrated to `deletedAt` in [Iteration 22](../../roadmap.md#iteration-22-standardize-soft-deletion-on-bed-and-plantinstance))._
...
## 5.2 Establishment & Germination Lifecycle (Nurseries) `[TARGET STATE (Pending Iterations [54](../../roadmap.md#iteration-54-calculate-germination-rates-dynamically-in-domain) & [74](../../roadmap.md#iteration-74-support-null-coordinates-for-nursery-establishments))]`
...
1. **Direct Establishment (`direct`):**
   ...
   - **Double-Deduction and Movement Protection [CRITICAL LOGICAL RULE]:** Saving or moving an existing layout (dragging coordinates) fires the `BedLayoutSaved` event. To prevent false seed depletions during mere spatial reorganization, the layout payload MUST explicitly segregate `addedInstances` (newly placed plants) from `movedInstances` (re-positioned existing plants). The stock deduction worker ONLY decrements inventories for IDs listed inside the `addedInstances` array. Additionally, the worker is backed by unique event-id idempotency checks inside Redis to prevent double-deductions during Kafka retries.
...
## 5.3 Transplant Workflow `[TARGET STATE (Pending [Iteration 75](../../roadmap.md#iteration-75-implement-transplant-api-with-spatial-validation))]`
...
## 5.4 Soft Delete Rule & Temporal Space Liberation `[TARGET STATE (Pending Iterations [22](../../roadmap.md#iteration-22-standardize-soft-deletion-on-bed-and-plantinstance) & [56](../../roadmap.md#iteration-56-build-2d-euclidean-proximity-calculator))]`
